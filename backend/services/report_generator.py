...
def safe_format_number(val, fmt=None, default="0"):
    try:
        if val is None:
            return default
        num = float(val)
...
env.filters['safe_num'] = safe_format_number
...
def generate_html_reports(analysis_data: Dict) -> Dict[str, str]:
    ...
    raw_ats = analysis_data.get('ATS_score') if analysis_data.get('ATS_score') is not None else analysis_data.get('ats_score', 0)
    overall_score = _safe_float(raw_ats, 0.0)
    ...
    def pct(score, max_score):
        max_val = max(1.0, float(max_score))
        return min(100, max(0, round(_safe_float(score) / max_val * 100)))
    ...
    def to_dict(item):
        if isinstance(item, dict):
            return item
        return item.model_dump() if hasattr(item, 'model_dump') else getattr(item, '__dict__', {})

    detailed_feedback = [to_dict(fb) for fb in raw_feedback]

    high_priority   = [fb for fb in detailed_feedback
                       if str(fb.get('severity_level', '')).lower() in ('critical', 'high')]
    ...
    if not isinstance(svd_raw, dict):
        svd_raw = {}

    validated_skills   = svd_raw.get('validated', []) if isinstance(svd_raw.get('validated'), list) else []
    unvalidated_skills = svd_raw.get('unvalidated', []) if isinstance(svd_raw.get('unvalidated'), list) else []
    total_skills       = int(_safe_float(svd_raw.get('total', len(validated_skills) + len(unvalidated_skills))))
    validated_count    = int(_safe_float(svd_raw.get('validated_count', len(validated_skills))))
    validation_pct     = _safe_float(svd_raw.get('validation_pct', 0.0))
    ...
    if jd_raw:
        reports['jd_report'] = env.get_template('quick_actions.html').render(**context)

    reports['recommendations'] = env.get_template('jd_comparison.html').render(**context)
